...
class RequestTest extends \arthur\test\Unit 
{
	...
	{
		$_SERVER['argv'] = array('/path/to/arthur.php', 'one', 'two');
		$request = new Request();

		$expected = '/path/to/arthur.php';
	...
	{
		$request = new Request(array('args' => array('/path/to/arthur.php', 'wrong')));

		$expected = array('/path/to/arthur.php', 'wrong');
		$result   = $request->argv;
		$this->assertEqual($expected, $result);

		$_SERVER['argv'] = array('/path/to/arthur.php');
	...
	public function testConstructWithConfigArgs() 
	{
		...
		$request = new Request(array(
			'env'  => array('script' => '/path/to/arthur.php'),
